...
public class BatterySensor {
	...
	private BatteryReading lastReading;
	...
	public void dataReady(BatteryReading reading) {
		listenerMutex.lock();
	...
	public class BatteryTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			lastReading = extractBatteryData(batteryStatus);
			dataReady(lastReading);
		...
		private BatteryReading extractBatteryData(Intent batteryStatus) {
			...
			BatteryReading reading = new BatteryReading(System.currentTimeMillis(), batteryPct, isCharging, usbCharge, acCharge, temp, volt, health);
