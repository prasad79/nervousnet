...
public class IfThisThenThatActivity extends Activity {
	...
	private Spinner spinner_Sensor, spinner_Stage1, spinner_Stage1Part2, spinner_TimeValue, spinner_TimeUnit;
	private Spinner spinner_then_stage1, spinner_then_stage2, spinner_then_TimeUnit, spinner_then_TimeValue;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		spinner_Sensor.setOnItemSelectedListener(new SensorSpinnerOnItemSelectedListener());
		spinner_then_stage1.setOnItemSelectedListener(new ThenStage1SpinnerOnItemSelectedListener());
	...
	private void updateTimeUnitSpinner() {
		...
		ArrayAdapter<String> adapter_TimeUnit = new ArrayAdapter<String>(this, R.layout.if_spinners, timeUnitArray);
	...
	private void updateTimeValueSpinner() {
		...
		ArrayAdapter<String> adapter_TimeValues = new ArrayAdapter<String>(this, R.layout.if_spinners, timeValuesArray);
	...
	private void updateStage1Part2Spinner() {
		...
		ArrayAdapter<String> adapter_IfStage1Part2 = new ArrayAdapter<String>(this, R.layout.if_spinners,
				ifStage1Part2Array);
	...
	private ArrayList<String> getThenStage1Array() {
		...
		String list[] = { "Vibrate", "Play Notification", "Flash Screen", "Turn off Phone", "Start App",
				"Call Contact" };
	...
	private ArrayList<String> getThenStage2Array() {
		ArrayList<String> result_list = new ArrayList<String>();
		String chosenThenStage1 = (String) spinner_then_stage1.getSelectedItem();
		if (chosenThenStage1.equals("Play Notification")) {
			String list[] = { "Soft Ringing", "Heavy Ringing", "Sound of the Sun", "Hats off", "Candlelight",
					"Stars Align" };
			result_list.addAll(Arrays.asList(list));
	...
	private void updateSensorSpinner() {
		...
		final ArrayAdapter<String> adapter_Sensor = new ArrayAdapter<String>(this, R.layout.if_spinners, sensorArray);
	...
	private void updateStage1Spinner() {
		...
		ArrayAdapter<String> adapter_IfStage1 = new ArrayAdapter<String>(this, R.layout.if_spinners, ifStage1Array);
	...
	private void updateThenTimeUnitSpinner() {
		...
		ArrayList<String> timeUnitArray = getTimeUnitArray();
		ArrayAdapter<String> adapter_TimeUnit = new ArrayAdapter<String>(this, R.layout.then_spinners, timeUnitArray);
	...
	private void updateThenTimeValueSpinner() {
		...
		ArrayList<String> timeValuesArray = getTimeValuesArray();
		ArrayAdapter<String> adapter_TimeValues = new ArrayAdapter<String>(this, R.layout.then_spinners,
				timeValuesArray);
	...
	private void updateThenStage1Spinner() {
		...
		ArrayAdapter<String> adapter_thenStage1 = new ArrayAdapter<String>(this, R.layout.then_spinners,
				thenStage1Array);
	...
	private void updateThenStage2Spinner() {
		...
		ArrayAdapter<String> adapter_thenStage2 = new ArrayAdapter<String>(this, R.layout.then_spinners,
				thenStage2Array);
	...
	private void updateThenForTextView() {
		...
		String hideThenForText_lsit[] = { "Start App", "Call Contact", "Turn off Phone" };
	...
	private ArrayList<String> getIfStage1Array() {
		...
		String aboveBelowlist[] = { "Light", "Temperature", "Humidity", "Light", "Battery", "Atmospheric Pressure",
		...
		if (aboveBelowSensorsArray.contains(chosenSensor)) {
			...
			itemsArray.addAll(Arrays.asList(list));
		} else if (chosenSensor.equals("Accelerometer") || chosenSensor.equals("Gyroscope")) {
			String list[] = { "is resting (low sensitivity)", "is resting (high sensitivity)", "is shaken heavily",
					"is shaken softly" };
			itemsArray.addAll(Arrays.asList(list));
		}
		return itemsArray;
	...
	private ArrayList<String> getIfStage1Part2Array() {
		...
		if (chosenSensor.equals("Temperature")) {
			...
			for (double i = 0; i <= 1.2; i += 0.1) {
				...
				itemsArray.add(new StringBuilder(i + "").subSequence(0, 3) + " lux");
			}
		}
		return itemsArray;
	}
	...
	private ArrayList<String> getSensorArray() {
		...
		String list[] = { "Accelerometer", "Light", "Temperature", "Humidity", "Gyroscope", "Light", "Proximity",
				"Battery", "Atmospheric Pressure", "Altitude", "Sound Level", "Date/Time" };
	...
	private ArrayList<String> getTimeUnitArray() {
		...
		String chosenSensor = (String) spinner_Sensor.getSelectedItem();
		if (!chosenSensor.equals("Date/Time")) {
			String list[] = { "seconds", "minutes", "hours", "days", "weeks", "years", "centuries" };
			itemsArray.addAll(Arrays.asList(list));
		}
		return itemsArray;
	}

	private final class SensorSpinnerOnItemSelectedListener implements OnItemSelectedListener {
		@Override
	...
	private final class ThenStage1SpinnerOnItemSelectedListener implements OnItemSelectedListener {
		@Override
		public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
			updateThenStage2Spinner();
			updateThenForTextView();
