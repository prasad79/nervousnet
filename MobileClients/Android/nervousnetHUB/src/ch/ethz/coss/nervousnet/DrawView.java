...
public class DrawView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(centerX, centerY, Math.min(maxW, maxH) * 0.18f, paintCircles);

		centerX = maxW * 0.6f;
		centerY = maxH * 0.68f;
		canvas.drawCircle(centerX, centerY, Math.min(maxW, maxH) * 0.3f, paintCircles);
		...
		centerX = maxW * 0.2f;
		centerY = maxH * 0.1f;
		canvas.drawCircle(centerX, centerY, Math.min(maxW, maxH) * 0.25f, paintCircles);
	...
	private void drawCircleNode(Canvas canvas, float centerX, float centerY, float r, double angle) {
		drawNodeEdge(canvas, (float) (centerX + (r * Math.cos(angle))), (float) (centerY - (r * Math.sin(angle))),
				centerX, centerY);
	}

	private void drawNodeEdge(Canvas canvas, float x1, float y1, float x2, float y2) {
