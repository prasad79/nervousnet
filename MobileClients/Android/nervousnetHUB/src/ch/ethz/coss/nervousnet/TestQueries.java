...
public class TestQueries {
	...
	/*
	 * class joinlp{ public Float lvalue; public Float pvalue; public Long
	 * tsofvalue;
	 * 
	 * joinlp(Float a,Float b,Long c) { lvalue = a; pvalue = b; tsofvalue = c; }
	 * }
	 */
	 ...
	 * 
	 * SensorQueriesLight light = new SensorQueriesLight(1, Long.MAX_VALUE,
	 * getFilesDir()); SensorQueriesProximity proximity = new
	 * SensorQueriesProximity(1, Long.MAX_VALUE, getFilesDir()); //for
	 * bottom10data ArrayList<SensorDescLight> light10 = light.getBottomK(10);
	 * ArrayList<SensorDescProximity> proximity10 = proximity.getBottomK(10);
	 * 
	 * ArrayList<joinlp> joinedlp = new ArrayList<joinlp>();
	 * 
	 * for(SensorDescLight i : light10) { Long currTime = i.getTimestamp(); Long
	 * dummy = Long.MAX_VALUE; Float value = (float) 0; Long timestamp = (long)
	 * 0; int k =0; int best_index = 0; for(SensorDescProximity j : proximity10)
	 * {
	 * 
	 * Long tempTime = j.getTimestamp(); Long diff =
	 * Math.abs(tempTime-currTime); if(diff < dummy) { dummy = diff; value =
	 * j.getValue(); timestamp = tempTime; best_index = k; } k++; }
	 * 
	...
	public void minProximity() {
		...
		if (sensorQ_Proximity.containsReadings()) {
			...
			Log.d(DEBUG_TAG,
					"Minimum Proximity: " + minProxDesc.getProximity() + "\nat " + getDate(minProxDesc.getTimestamp()));
		} else {
			Log.d(DEBUG_TAG, "No Data Found");
		}
	}
	...
	public void maxProximity() {
		...
		if (sensorQ_Prox.containsReadings()) {
			...
			Log.d(DEBUG_TAG,
					"Maximum Prox: " + maxProxDesc.getProximity() + "\nat " + getDate(maxProxDesc.getTimestamp()));
		} else {
			Log.d(DEBUG_TAG, "No Data Found ");
		}
	}
	...
	public void maxBattery() {
		...
		if (sensorQ_Batteries2.containsReadings()) {
			...
			Log.d(DEBUG_TAG,
					"Max Battery: " + maxBatDesc.getBatteryPercent() + "\nat " + getDate(maxBatDesc.getTimestamp()));
		} else {
			Log.d(DEBUG_TAG, "No Data Found");
		}
	}
	...
	public void minBattery() {
		...
		if (sensorQ_Batteries.containsReadings()) {
			...
			Log.d(DEBUG_TAG, "Minimum Battery: " + minBatDesc.getBatteryPercent() + "\nat "
					+ getDate(minBatDesc.getTimestamp()));
		} else {
			Log.d(DEBUG_TAG, "No Data Found");
		}
	}
