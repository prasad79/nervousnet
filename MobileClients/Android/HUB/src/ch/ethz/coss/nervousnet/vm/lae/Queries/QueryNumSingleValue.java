...
public abstract class QueryNumSingleValue<G extends SensorDescSingleValue> extends QueryNum<G> {
	...
	public QueryNumSingleValue(long timestamp_from, long timestamp_to, File file) {
	...
	{// return data within the range
		...
		// after that loop through all the elements and check if value within
		// range
	...
	@Override
	public ArrayList<Float> getRmsError(ArrayList<Float> comp) // sum of squared
																// diff between
																// same sensor
																// readings at
																// different
	...
	public G getRankSmallest(int k) {// rank k from acsending order
		// 1 based rank
		try {
			Comparator<G> comparator = new SmallestFirstComparator();// ascending
																		// order
			PriorityQueue<G> prioQueue = new PriorityQueue<G>(3, comparator);
	...
	public G getRankLargest(int k) {// largest elelment of rank k
		// 1 based rank
		try {
			Comparator<G> comparator = new LargestFirstComparator();// descending
																	// order
			PriorityQueue<G> prioQueue = new PriorityQueue<G>(3, comparator);// more
																				// general
																				// than
																				// a
																				// queue
			...
			for (SensorData sensorData : list) {
				prioQueue.add(createSensorDescSingleValue(sensorData)); // add
																		// elements
																		// to
																		// the
																		// queue
	...
	@Override
	public ArrayList<Float> getCorrelation(ArrayList<G> comp, ArrayList<G> comp1)// single
																					// value
																					// sensors
	{
	 ...
	 * 
	 * public ArrayList<G> getSensorDescriptorList() { ArrayList<G> descList =
	 * new ArrayList<G>(); for (SensorData sensorData : list) {
	 * descList.add(createSensorDescSingleValue(sensorData)); } return descList;
	 * }
	 */
