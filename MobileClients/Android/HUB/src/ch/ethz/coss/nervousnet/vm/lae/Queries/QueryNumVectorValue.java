package ch.ethz.coss.nervousnet.vm.lae.Queries;
...
import ch.ethz.coss.nervousnet.sensors.SensorDescVectorValue;
...
public abstract class QueryNumVectorValue<G extends SensorDescVectorValue> extends QueryNum<G> {
	...
	@Override
	public ArrayList<G> getTimeRange(ArrayList<G> desc_list, ArrayList<Float> s, ArrayList<Float> e) // send
																										// to
																										// function
																										// list
																										// of
	...
	// specified.
	{
		...
		try {
			for (int i = 0; i < desc_list.size(); i++) // loop through the data
			{
				...
				int flag = 0;
				for (int j = 0; j < temp.size(); j++) // all values of all
														// dimensions lesser and
														// greater than start
														// and end
				{
	...
	public ArrayList<Float> getRmsError(ArrayList<Float> comp) {
		...
		try {
			...
			int s = 0;
			for (SensorData sensorData : list) {
				G sensDesc = createSensorDescVectorValue(sensorData); // loop
																		// over
																		// the
																		// sensor
																		// data,get
																		// the
																		// object
				ArrayList<Float> temp = new ArrayList<Float>(); // temporary
																// data
	...
	public ArrayList<Float> var() {
		...
		try {
			...
			for (SensorData sensorData : list) {
				...
				{
					float temptemp = average.get(i) + temp.get(i); // add
																	// current
																	// data to
																	// the
																	// existing
	...
	public G getMaxValue() { // add all three values and find the maximum
		...
		try {
			float maxAverage = 0;
			for (SensorData sensorData : list) {
				G sensDesc = createSensorDescVectorValue(sensorData); // get
																		// sensor
																		// data
				ArrayList<Float> value = new ArrayList<Float>();
				value = sensDesc.getValue(); // get arraylist of values of
												// sensor(3 values generally)
				float newAverage = 0;
	...
	public ArrayList<Float> getMedian() {
		...
		/*
		 * public static double median(double[] m) { int middle = m.length/2; if
		 * (m.length%2 == 1) { return m[middle]; } else { return (m[middle-1] +
		 * m[middle]) / 2.0; } }
		 */
	...
	public ArrayList<Float> getSum() { // find the average of all the values
		ArrayList<Float> sum = new ArrayList<Float>(); // 0-> avg of x and so
														// on...
		try {
			for (SensorData sensorData : list) {
				G sensDesc = createSensorDescVectorValue(sensorData); // loop
				...
				for (int i = 0; i < temp.size(); i++) // for each x,z & z
				{
					float temptemp = sum.get(i) + temp.get(i); // add current
																// data to the
																// existing one
																// and replace
					sum.set(i, temptemp);
	...
	@Override
	public ArrayList<Float> getSumSquare() { // find the average of all the
												// values
		ArrayList<Float> sum = new ArrayList<Float>(); // 0-> avg of x and so
														// on...
		try {
			for (SensorData sensorData : list) {
				...
				for (int i = 0; i < temp.size(); i++) // for each x,z & z
				{
					float temptemp = (float) (sum.get(i) + Math.pow(temp.get(i), 2)); // add
																						// current
																						// data
																						// to
																						// the
	...
	@Override
	public ArrayList<Float> getMeanSquare() { // find the average of all the
												// values
		ArrayList<Float> sum = new ArrayList<Float>(); // 0-> avg of x and so
														// on...
		try {
	...
	public class SmallestFirstComparator implements Comparator<G> { // one
																	// object
																	// smaller
																	// than the
																	// other
	...
	public class LargestFirstComparator implements Comparator<G> { // one object
																	// larger
																	// than the
																	// other
	...
	public ArrayList<Float> getCorrelation(ArrayList<G> comp, ArrayList<G> comp1) {
		...
		try {
			...
			ArrayList<ArrayList<Float>> a = new ArrayList<ArrayList<Float>>();// centered
																				// data
			...
			// now we have a and b centered value and avg and avg1 their
			// averages
		...
		try {
			...
			for (SensorData sensorData : list) {
				...
				for (int i = 0; i < temp.size(); i++) // for each x,z & z
				{
					...
					temp.add(i, temptemp); // now temp contains the
											// probabilities of x,y,z
				}
