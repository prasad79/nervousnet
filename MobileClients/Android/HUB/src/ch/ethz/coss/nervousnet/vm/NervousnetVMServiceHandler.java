...
public class NervousnetVMServiceHandler {
	...
	protected void scheduleSensor(final long sensorId, final Context context, final SensorEventListener listener) {
		...
		run = new Runnable() {
			...
			public void run() {
				...
				// Toast.makeText(NervousnetVMServiceHandler.this, "Finished
				// Running Schedule Sensor thread", Toast.LENGTH_LONG).show();
	...
	public void startStopSensorService(boolean on, Context context) {
		if (on) {
			...
			// sc.getInitialSensorCollectStatus(SensorDescBLEBeacon.SENSOR_ID);
			// if (scs.isCollect()) {
