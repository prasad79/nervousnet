...
import ch.ethz.coss.nervousnet.utils.UnsignedArithmetic;
...
public class BLEBeaconRecord {
	...
	public BLEBeaconRecord(long tokenDetectTime, BluetoothDevice device, int rssi, byte[] scanRecord) {
		...
		this.major = (UnsignedArithmetic.upcastToInt(scanRecord[25]) << 8)
				| (UnsignedArithmetic.upcastToInt(scanRecord[26]));
		this.minor = (UnsignedArithmetic.upcastToInt(scanRecord[27]) << 8)
				| (UnsignedArithmetic.upcastToInt(scanRecord[28]));
